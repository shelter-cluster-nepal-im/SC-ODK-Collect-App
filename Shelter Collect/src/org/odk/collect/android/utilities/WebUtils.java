...
public final class WebUtils {
	...
	public static final List<AuthScope> buildAuthScopes(String host) {
		...
		asList.add(a);
		a = new AuthScope(host, 8080, null, AuthPolicy.BASIC);
		asList.add(a);
	...
	public static final void addCredentials(String userEmail, String password,
			String host) {
		HttpContext localContext = Collect.getInstance().getHttpContext();
		...
		try {
			...
			if (!entity.getContentType().getValue().toLowerCase()
				...
				String error = "ContentType: "
						...
						+ "returned from: " + u.toString() + " is not text/xml";
				Log.e(t, error);
				return new DocumentFetchResult(error, 0);
			}
